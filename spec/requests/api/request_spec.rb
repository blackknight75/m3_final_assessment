...
describe "Items", type: :request do
  ...
  expect(items[:items].first[:name]).to eq("stuff")
  expect(items[:items].last[:name]).to eq("again stuff")
  end
  ...
  it "returns a single item" do
    ...
    expect(item[:item][:name]).to eq("stuff")
    expect(item[:item][:description]).to eq("some stuff")
  end
